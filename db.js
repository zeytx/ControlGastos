...
const FinanceDB = (() => {
  ...
  const CARD_SOURCE_TYPES = new Set(['manual', 'legacy', 'recurring', 'system', 'statement-import']);
  ...
  const DEFAULT_SETTINGS = {
    ...
    monthlyBudget: 0,
    categoryBudgets: {},
    exchangeRates: { USD: 3.75 },
    notifications: { cardDueEnabled: true, daysBefore: 3, lastNotifiedKey: '' },
  ...
  async function openRawDB() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        ensureStore(db, STORE_NAMES.reconciliations, { keyPath: 'id' });
  ...
  function normalizeSettings(input = {}) {
    ...
    const rawBudgets = parseJson(merged.categoryBudgets, {});
    merged.categoryBudgets = Object.entries(rawBudgets).reduce((acc, [categoryId, amount]) => {
      const value = roundAmount(amount);
      if (categoryId && value > 0) acc[categoryId] = value;
      return acc;
    }, {});

    const rawRates = parseJson(merged.exchangeRates, {});
    merged.exchangeRates = Array.from(CURRENCIES)
      .filter((code) => code !== BASE_CURRENCY)
      .reduce((acc, code) => {
        const value = toNumber(rawRates[code], DEFAULT_SETTINGS.exchangeRates[code] || 1);
        acc[code] = value > 0 ? Math.round(value * 10000) / 10000 : DEFAULT_SETTINGS.exchangeRates[code] || 1;
        return acc;
      }, {});

    const rawNotifications = parseJson(merged.notifications, {});
    merged.notifications = {
      cardDueEnabled: rawNotifications.cardDueEnabled !== false,
      daysBefore: Math.max(1, Math.min(10, parseInt(rawNotifications.daysBefore, 10) || 3)),
      lastNotifiedKey: String(rawNotifications.lastNotifiedKey || '').trim()
    };
  ...
  function normalizeAccount(input = {}, existing = null) {
    ...
    const currency = CURRENCIES.has(input.currency)
      ? input.currency
      : CURRENCIES.has(existing?.currency) ? existing.currency : BASE_CURRENCY;
    return {
  ...
  function normalizeTransaction(input = {}, existing = null) {
    ...
    const normalized = {
      ...
      installmentAmount: type === 'card_charge'
        ...
        : 0,
      // Solo se usa en transferencias entre cuentas de distinta moneda:
      // "amount" sale de la cuenta origen y "toAmount" entra a la destino.
      toAmount: roundAmount(input.toAmount ?? existing?.toAmount ?? 0),
  ...
  function getRecurringProjectionBounds(recurring, transactions) {
    ...
    const recurringDates = recurring
      ...
      .filter(Boolean)
      .sort(compareDate);
    // Si declaraste que tu sueldo empieza en marzo, los ciclos deben existir
    // desde marzo aunque tu movimiento mas viejo sea de la semana pasada.
    const earliestAuthored = authoredDates[0] || '';
    const earliestRecurring = recurringDates[0] || '';
    const first = earliestAuthored && earliestRecurring
      ? (compareDate(earliestAuthored, earliestRecurring) <= 0 ? earliestAuthored : earliestRecurring)
      : earliestAuthored || earliestRecurring || today;
    ...
    sortByDateAsc(transactions).forEach((transaction) => {
      ...
      if (transaction.type === 'expense') {
        ...
        if (transaction.toAccountId) {
          const received = transaction.toAmount > 0 ? transaction.toAmount : transaction.amount;
          balances[transaction.toAccountId] = roundAmount((balances[transaction.toAccountId] || 0) + received);
        }
  ...
  function convertToBase(amount, currency, settings) {
    const value = roundAmount(amount);
    if (!currency || currency === BASE_CURRENCY) return value;
    const rate = toNumber(settings?.exchangeRates?.[currency], 1);
    return roundAmount(value * (rate > 0 ? rate : 1));
  }

  function getAccountCurrency(accountId, accounts) {
    const account = accounts.find((item) => item.id === accountId);
    return account?.currency || BASE_CURRENCY;
  }

  // ===== Clasificador local de categorias =====
  // Aprende de lo que el usuario ya categorizo. Sin IA: es mas rapido,
  // funciona sin conexion y acierta mas en los comercios de siempre.

  const CLASSIFIER_STOPWORDS = new Set([
    'de', 'del', 'la', 'el', 'los', 'las', 'en', 'por', 'para', 'con', 'un', 'una',
    'pago', 'compra', 'cuota', 'sa', 'sac', 'srl', 'eirl', 'peru', 'lima'
  ]);
  ...
  function descriptionTokens(value) {
    return normalizeDescription(value)
      .split(' ')
      .filter((token) => token.length >= 3 && !CLASSIFIER_STOPWORDS.has(token) && !/^\d+$/.test(token));
  }

  function buildCategoryClassifier(transactions) {
    const exact = new Map();
    const byToken = new Map();

    transactions
      .filter((item) => item.categoryId && ['expense', 'income', 'card_charge'].includes(item.type))
      .forEach((item) => {
        const key = normalizeDescription(item.description);
        if (key) {
          if (!exact.has(key)) exact.set(key, new Map());
          const counts = exact.get(key);
          counts.set(item.categoryId, (counts.get(item.categoryId) || 0) + 1);
        }

        descriptionTokens(item.description).forEach((token) => {
  ...
  function pickTopCategory(counts) {
    ...
    return best ? { categoryId: best, count: bestCount, confidence: bestCount / total } : null;
  }

  function suggestCategory(description, transactions, { type = 'expense' } = {}) {
    const classifier = transactions instanceof Map || !Array.isArray(transactions)
      ? transactions
      : buildCategoryClassifier(transactions);
    if (!classifier?.exact) return null;

    const key = normalizeDescription(description);
    if (!key) return null;

    const exactMatch = pickTopCategory(classifier.exact.get(key));
    if (exactMatch) return { ...exactMatch, source: 'exact' };

    // Sin coincidencia exacta se vota entre los tokens ("metro" pesa aunque
    // el resto de la descripcion cambie cada vez).
    const votes = new Map();
    descriptionTokens(description).forEach((token) => {
      const counts = classifier.byToken.get(token);
      if (!counts) return;
      counts.forEach((count, categoryId) => {
    ...
    const tokenMatch = pickTopCategory(votes);
    if (tokenMatch && tokenMatch.confidence >= 0.5) {
      return { ...tokenMatch, source: 'token' };
    }

    return null;
  }

  function normalizeReconciliation(input = {}, existing = null) {
    const now = new Date().toISOString();
    return {
      id: String(input.id || existing?.id || createId('rec-st')).trim(),
      cardId: String(input.cardId || existing?.cardId || '').trim(),
      statementCycleKey: String(input.statementCycleKey || existing?.statementCycleKey || '').trim(),
      reconciledAt: input.reconciledAt || existing?.reconciledAt || now,
      statementTotal: roundAmount(input.statementTotal ?? existing?.statementTotal ?? 0),
      appTotal: roundAmount(input.appTotal ?? existing?.appTotal ?? 0),
      difference: roundAmount(input.difference ?? existing?.difference ?? 0),
      matchedCount: Math.max(0, parseInt(input.matchedCount ?? existing?.matchedCount ?? 0, 10) || 0),
      missingCount: Math.max(0, parseInt(input.missingCount ?? existing?.missingCount ?? 0, 10) || 0),
      extraCount: Math.max(0, parseInt(input.extraCount ?? existing?.extraCount ?? 0, 10) || 0),
      notes: String(input.notes ?? existing?.notes ?? '').trim().slice(0, 300),
      createdAt: existing?.createdAt || input.createdAt || now,
      updatedAt: now
    };
  ...
  function buildBudgetStatus(settings, cycle, transactions, statements, categories = [], accounts = []) {
    const limit = roundAmount(settings.monthlyBudget);
    const categoryBudgets = settings.categoryBudgets || {};
    const hasCategoryBudgets = Object.keys(categoryBudgets).length > 0;

    if ((!limit && !hasCategoryBudgets) || !cycle) {
      return { enabled: false, limit, spent: 0, remaining: 0, ratio: 0, overspent: false, byCategory: [] };
    }

    const cycleTransactions = transactions.filter((item) => item.budgetCycleId === cycle.id);
    const spendTransactions = cycleTransactions.filter((item) =>
      ['expense', 'debt_payment'].includes(item.type)
    );

    const amountInBase = (item) => {
      const currency = getAccountCurrency(item.fromAccountId, accounts);
      return convertToBase(item.amount, currency, settings);
    };

    const cashSpend = spendTransactions.reduce((sum, item) => sum + amountInBase(item), 0);
    const cardCharges = cycleTransactions.filter((item) => item.type === 'card_charge');
    ...
    spendTransactions.concat(cardCharges).forEach((item) => {
      if (!item.categoryId) return;
      const value = item.type === 'card_charge' ? roundAmount(item.amount) : amountInBase(item);
      spentByCategory.set(item.categoryId, roundAmount((spentByCategory.get(item.categoryId) || 0) + value));
    });

    const categoryNames = new Map(categories.map((item) => [item.id, item.name]));
    const byCategory = Object.entries(categoryBudgets)
      .map(([categoryId, categoryLimit]) => {
        const categorySpent = roundAmount(spentByCategory.get(categoryId) || 0);
        return {
          categoryId,
          name: categoryNames.get(categoryId) || categoryId,
          limit: roundAmount(categoryLimit),
          spent: categorySpent,
          remaining: roundAmount(categoryLimit - categorySpent),
          ratio: categoryLimit > 0 ? Math.min(1, categorySpent / categoryLimit) : 0,
          overspent: categorySpent > categoryLimit
        };
      })
    ...
    return {
      ...
      overspent: limit > 0 && spent > limit,
      byCategory
    };
  }

  function calculateNetWorth(accounts, balances, cardSummaries, debts, settings) {
    const liquid = roundAmount(
      accounts
        .filter((account) => account.includeInNetWorth && !account.archived)
        .reduce((sum, account) => sum + convertToBase(balances[account.id] || 0, account.currency, settings), 0)
    );
    const cardDebt = roundAmount(
      cardSummaries
        .filter((card) => !card.archived)
        .reduce((sum, card) => sum + card.currentDebt, 0)
    );
    const otherDebt = roundAmount(
      debts
        .filter((item) => !item.archived && !item.legacyMigratedToCardId)
        .reduce((sum, item) => sum + item.outstandingAmount, 0)
    );

    const byCurrency = accounts
      .filter((account) => account.includeInNetWorth && !account.archived)
      .reduce((acc, account) => {
        const code = account.currency || BASE_CURRENCY;
        acc[code] = roundAmount((acc[code] || 0) + (balances[account.id] || 0));
        return acc;
      }, {});
    ...
    return {
      ...
      cardDebt,
      otherDebt,
      total: roundAmount(liquid - cardDebt - otherDebt),
      byCurrency,
      baseCurrency: BASE_CURRENCY
    };
  }

  /** Patrimonio neto al cierre de cada ciclo, para el grafico de evolucion. */
  function buildNetWorthHistory(cycles, accounts, transactions, cards, settings) {
    const today = getToday();
    const ordered = [...cycles].sort((a, b) => compareDate(a.startDate, b.startDate));

    return ordered
      .filter((cycle) => compareDate(cycle.startDate, today) <= 0)
      .map((cycle) => {
        const cutoffSource = compareDate(cycle.endDate, today) < 0 ? cycle.endDate : today;
        const cutoffDate = addDays(cutoffSource, 1);
        const balances = calculateBalances(accounts, transactions, cutoffDate);
        const liquid = roundAmount(
          accounts
            .filter((account) => account.includeInNetWorth && !account.archived)
            .reduce((sum, account) => sum + convertToBase(balances[account.id] || 0, account.currency, settings), 0)
        );
        const statements = deriveStatements(cards, transactions, ordered, cutoffDate);
        const cardDebt = roundAmount(
        ...
        return {
          ...
          liquid,
          cardDebt,
          net: roundAmount(liquid - cardDebt)
        };
      })
      .slice(-12);
  }
  ...
  async function saveTransaction(payload) {
    ...
    // Transferencias entre monedas distintas: hay que decir cuanto llega,
    // porque el tipo de cambio del dia no tiene por que ser el de la app.
    if (['transfer', 'goal_contribution'].includes(record.type) && record.toAccountId) {
      const accounts = await getAccounts();
      const fromCurrency = getAccountCurrency(record.fromAccountId, accounts);
      const toCurrency = getAccountCurrency(record.toAccountId, accounts);
      if (fromCurrency !== toCurrency) {
        if (!(record.toAmount > 0)) {
          throw new Error(`Esta transferencia va de ${fromCurrency} a ${toCurrency}: indica cuanto llega a la cuenta destino.`);
        }
      } else {
        record.toAmount = 0;
      }
  ...
  async function exportBackup() {
    ...
    return {
      ...
      reconciliations: await getReconciliations()
    };
  }
  ...
  function normalizeBackup(parsed) {
    ...
    const collections = ['categories', 'accounts', 'goals', 'debts', 'recurring', 'cards', 'transactions', 'financial_cycles', 'reconciliations'];
    ...
    return {
      ...
      [STORE_NAMES.reconciliations]: (parsed.reconciliations || []).map((item) => normalizeReconciliation(item, item))
    };
  }
  ...
  async function getFinanceSnapshot() {
    ...
    const netWorth = calculateNetWorth(accounts, balances, cardSummaries, derivedDebts, settings);
    const netWorthHistory = buildNetWorthHistory(cycleSummaries, accounts, transactions, cards, settings);
    const budget = buildBudgetStatus(settings, currentCycle, transactions, statements, categories, accounts);
    const reconciliations = (await getAll(STORE_NAMES.reconciliations)).map((item) =>
      normalizeReconciliation(item, item)
    );
    const categoryClassifier = buildCategoryClassifier(transactions);
  ...
  async function getReconciliations() {
    await initDB();
    return (await getAll(STORE_NAMES.reconciliations)).map((item) => normalizeReconciliation(item, item));
  }

  /** Compara los movimientos que la IA leyo del estado de cuenta contra los registrados. */
  function compareStatementMovements(statementMovements, appTransactions, { toleranceDays = 5 } = {}) {
    const pending = appTransactions.map((item, index) => ({ item, index, used: false }));
    const matched = [];
    const missing = [];

    (statementMovements || []).forEach((movement) => {
      const amount = roundAmount(movement.amount);
      const date = normalizeDate(movement.date);
      const needle = normalizeDescription(movement.description);

      const candidates = pending.filter((entry) => !entry.used && Math.abs(entry.item.amount - amount) < 0.02);

      const scored = candidates
        .map((entry) => {
          const entryDate = normalizeDate(entry.item.purchaseDate || entry.item.date);
          const dayGap = Math.abs(
            (new Date(`${entryDate}T12:00:00`) - new Date(`${date}T12:00:00`)) / 86400000
          );
          const entryText = normalizeDescription(entry.item.description);
          const sharedTokens = descriptionTokens(needle).filter((token) => entryText.includes(token)).length;
          return { entry, dayGap, sharedTokens };
        })
        .filter((option) => option.dayGap <= toleranceDays)
        .sort((a, b) => b.sharedTokens - a.sharedTokens || a.dayGap - b.dayGap);

      if (scored.length) {
        scored[0].entry.used = true;
        matched.push({ movement, transactionId: scored[0].entry.item.id });
        return;
      }

      missing.push({ ...movement, amount, date });
    });

    const extra = pending.filter((entry) => !entry.used).map((entry) => entry.item);

    const statementTotal = roundAmount(
      (statementMovements || []).reduce((sum, item) => sum + roundAmount(item.amount), 0)
    );
    const appTotal = roundAmount(appTransactions.reduce((sum, item) => sum + item.amount, 0));
    ...
    return {
      ...
      statementTotal,
      appTotal,
      difference: roundAmount(statementTotal - appTotal)
    };
  }

  async function reconcileStatement({ cardId, statementCycleKey, movements = [], notes = '' }) {
    await initDB();
    if (!cardId) throw new Error('Elige la tarjeta del estado de cuenta.');
    ...
    const appTransactions = transactions.filter(
      (item) =>
        item.cardId === cardId &&
        item.type === 'card_charge' &&
        (!statementCycleKey || item.statementCycleKey === statementCycleKey)
    );

    const comparison = compareStatementMovements(movements, appTransactions);
    const record = normalizeReconciliation({
      id: `recon-${cardId}-${statementCycleKey || 'sin-cierre'}`,
      cardId,
      statementCycleKey,
      statementTotal: comparison.statementTotal,
      appTotal: comparison.appTotal,
    ...
    await putMany(STORE_NAMES.reconciliations, [record]);
    return { ...comparison, record };
  }

  async function deleteReconciliation(id) {
    await initDB();
    await deleteOne(STORE_NAMES.reconciliations, id);
  }

  /** Da de alta de golpe los movimientos que estaban en el estado pero no en la app. */
  async function importMissingStatementMovements(cardId, movements = [], categoryId = 'other-expense') {
    await initDB();
    const transactions = await getTransactions();
    const classifier = buildCategoryClassifier(transactions);

    for (const movement of movements) {
      const suggestion = suggestCategory(movement.description, classifier);
      await saveTransaction({
        ...
        description: movement.description || 'Cargo del estado de cuenta',
        categoryId: suggestion?.categoryId || categoryId,
        installmentCount: 1,
        sourceType: 'statement-import',
        notes: 'Agregado al conciliar el estado de cuenta'
      });
    }
  ...
  return {
    ...
    importMissingStatementMovements,
